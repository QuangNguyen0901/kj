...
define( 'MTS_THEME_VERSION', '1.3.12' );
...
if (!function_exists( '_wp_render_title_tag' ) ) {
    function mts_render_title() {
        echo '<title>' . wp_title( '|', true, 'right' ) . '</title>';
    }
    add_action( 'wp_head', 'mts_render_title' );
}


/*-----------------------------------------------------------------------------------*/
/*  Remove check out fields　　Quang　
/*-----------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------*/
/*  remove required properties　　Quang　
/*-----------------------------------------------------------------------------------*/
...
if ( function_exists( 'add_theme_support' ) ) {
    ...
    add_image_size( 'product_gallery_thumbnail', 160, 160, true );
}
/*-----------------------------------------------------------------------------------*/
/*  Product page gallery thumbnail size
/*-----------------------------------------------------------------------------------*/
function mts_woocommerce_gallery_thumbnail_size() {
  return 'product_gallery_thumbnail';
}
add_filter( 'woocommerce_gallery_thumbnail_size', 'mts_woocommerce_gallery_thumbnail_size' , 24 );
...
if(!function_exists('mts_get_excluded_sidebars')){
...
if(!function_exists('mts_custom_sidebar')){
  function mts_custom_sidebar() {
      ...
      if ( is_page() || is_single() ) {
          ...
          $custom = get_post_meta( wc_get_page_id('shop'), '_mts_custom_sidebar', true );
          if ( !empty( $custom ) && array_key_exists( $custom, $wp_registered_sidebars ) || 'mts_nosidebar' == $custom ) $sidebar = $custom;
      }
...
function mts_load_footer_scripts() {
    ...
    //Lightbox
    wp_register_script( 'mtsprettyPhoto', get_template_directory_uri() . '/js/jquery.prettyPhoto.js', true );
    if ( ! empty( $mts_options['mts_lightbox'] ) ) {
        if ( mts_isWooCommerce() ) {
            if ( version_compare( WC()->version, '3.0.0', ">=" ) ) {
                wp_enqueue_script( 'mtsprettyPhoto' );
            } else {
    ...
    if ( mts_isWooCommerce() ) {
        if ( is_product() && version_compare( WC()->version, '3.0.0', ">=" ) && isset( $mts_options['mts_wc_product_gallery'] ) && '0' == $mts_options['mts_wc_product_gallery'] ) {
...
function mts_enqueue_css() {
    ...
    // Lightbox
    if ( !$is_single_prod && ! empty( $mts_options['mts_lightbox'] ) ) {
        wp_register_style( 'mtsprettyPhoto', get_template_directory_uri() . '/css/prettyPhoto.css', 'style' );
        wp_enqueue_style( 'mtsprettyPhoto' );
    }
      ...
      <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
          <div id="comment-<?php comment_ID(); ?>" itemscope itemtype="http://schema.org/UserComments">
              <div class="comment-author vcard">
                  ...
                  <?php printf( '<span class="fn"><i class="fa fa-user"></i><span>%s</span></span>', get_comment_author_link() ) ?>
...
if ( mts_isWooCommerce() ) {
    ...
    // Override woocommerce_show_product_images() function
  ...
  function woocommerce_show_product_images() {
      ...
      if ( version_compare( WC()->version, '3.0.0', ">=" ) ) {
          if ( isset( $mts_options['mts_wc_product_gallery'] ) && '1' === $mts_options['mts_wc_product_gallery'] ) {
              ...
              wc_get_template( 'single-product/product-image-slider.php' );
          }
      } else {
          wc_get_template( 'single-product/product-image-slider.php' );
      }
...
add_filter( 'woocommerce_related_products_columns', 'loop_columns' );
add_filter( 'woocommerce_cross_sells_columns', 'loop_columns' );
add_filter( 'woocommerce_up_sells_columns', 'loop_columns' );
...
// Remove WooCommerce cart total from default place, it's called within woocommerce/cart/cart.php instead
...
if( !function_exists('mts_show_product_loop_new_badge')){
...
if(!function_exists('mts_product_cat_tax_add_form_fields')){
...
if(!function_exists('mts_product_cat_tax_edit_form_fields')){
...
if(!function_exists('mts_create_update_product_cat_icon')){
...
if(!function_exists('mts_delete_product_cat_icon')){
...
if(!function_exists('mts_product_category_scripts')){
...
if(!function_exists('mts_attribute_tax_edit_form_fields')){
...
if(!function_exists('mts_create_update_attribute_tax_color')){
...
if(!function_exists('mts_delete_attribute_tax_color')){
...
if(!function_exists('mts_attribute_tax_picker')){
  function mts_attribute_tax_picker() {
    ...
    function mts_ajax_homepage_tabs() {
        mts_homepage_tab( $_POST['home_tab'], $_POST['term_slug'] );
        die();
    }
		if( !function_exists( 'mts_homepage_tab') ){
			function mts_homepage_tab( $tab, $slug ) {

					if ( 'new_products_tab' === $tab ) {
							$new_products_args = apply_filters( 'woocart_homepage_tab_args', array('posts_per_page' => 4, 'no_found_rows' => 1, 'post_status' => 'publish', 'post_type' => 'product', 'product_cat' => $slug ), $tab, $slug );
							...
							$top_rated_args = apply_filters( 'woocart_homepage_tab_args', array('posts_per_page' => 4, 'no_found_rows' => 1, 'post_status' => 'publish', 'post_type' => 'product', 'product_cat' => $slug ), $tab, $slug );
							...
							$best_sellers_args = apply_filters( 'woocart_homepage_tab_args', array( 'meta_key' => 'total_sales', 'orderby' => 'meta_value_num', 'posts_per_page' => 4, 'no_found_rows' => 1, 'post_status' => 'publish', 'post_type' => 'product', 'product_cat' => $slug ), $tab, $slug );
...
/*-----------------------------------------------------------------------------------*/
if(!function_exists('mts_get_post_template')){
...
function mts_correct_homepage_sections_import( $item, $key, $data ) {
    ...
    if ( 'mts_home_brands' !== $key && 'mts_custom_slider' !== $key && 'mts_custom_offers_slider' !== $key && 'mts_featured_product_categories' !== $key && 'mts_offers_page_filter_menu' !== $key) return $item;
    ...
    } elseif ( 'mts_home_brands' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_home_brand_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_home_brand_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_home_brand_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } elseif ( 'mts_custom_offers_slider' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_custom_offers_slider_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_custom_offers_slider_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_custom_offers_slider_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } elseif ( 'mts_featured_product_categories' === $key ) {

        foreach( $item as $k => $featured_product_cat ) {
            ...
            $product_cat_existing = get_term_by('name', $product_cat_name, 'product_cat');
            if ( array_key_exists( $product_cat_id, $data['terms']['product_cat'] ) ) {

                $new_item[ $k ]['mts_featured_product_category'] = $data['terms']['product_cat'][ $product_cat_id ].'|'.$product_cat_name;

            } else if ( $product_cat_existing ) {

                $new_item[ $k ]['mts_featured_product_category'] = $product_cat_existing->term_id.'|'.$product_cat_name;
            }
        }

    } else { // mts_offers_page_filter_menu

        foreach( $item as $i => $filter_menu_category ) {

            $product_cat_id = $filter_menu_category['mts_offers_page_filter_menu_category'];

            if ( array_key_exists( $product_cat_id, $data['terms']['product_cat'] ) ) {

                $new_item[ $i ]['mts_offers_page_filter_menu_category'] = $data['terms']['product_cat'][ $product_cat_id ];
            }
        }
    }
...
function mts_admin_bar_link() {
    ...
    if( current_user_can( 'edit_theme_options' ) ) {
        $wp_admin_bar->add_menu( array(
            ...
            'title' => __( 'Theme Options', 'mythemeshop' ),
